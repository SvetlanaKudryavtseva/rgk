...
<?if($arResult['ITEMS']):?>	
	...
	<?if (!$arParams['IS_AJAX']):?>
		...
		<div class="inner-block__title-wrapper">
			...
			<div class="inner-block__part--right">
				<a class="inner-block__link dark_link stroke-theme-hover right_link_block" href="<?=$arParams['RIGHT_LINK'];?>"><?=$arParams['RIGHT_TITLE'];?>
					<span class="inner-block__arrow">
			...
			foreach($arResult['ITEMS'] as $i => $arItem):?>
				...
				<div class="blog-list__wrapper <?=$itemWrapperClasses;?> <?=$itemWrapperClassesExt;?>">
					<div class="blog-list__item <?=$itemClasses?> bordered rounded-4 shadow-hovered shadow-no-border-hovered <?=($arItem['DISPLAY_PROPERTIES']['SALE_NUMBER']['VALUE'] || $bDiscountCounter ? 'blog-list__item--with-discount' : '');?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
						<?if($imageSrc):?>
							<div class="blog-list__item-image-wrapper blog-list__item-image-wrapper--TOP <?='blog-list__item-image-wrapper--'.$arParams['IMAGE_POSITION']?>">
						...
						<div class="blog-list__item-text-wrapper blog-list__item-text-wrapper--BOTTOM_RELATIVE <?='blog-list__item-text-wrapper--'.$arParams['TEXT_POSITION']?> flex-grow-1 flexbox">
							<?if($bShowSection):?>
